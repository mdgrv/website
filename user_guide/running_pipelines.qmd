...
Dependening on whether you want to run a workflow locally or on cloud infrastructure, using Seqera Platform (Nextflow Tower) or not, you will need to use different commands.
...
If you have access to a Seqera Platform instance, previously known as Nextflow Tower, in which a Compute Environment has already been set up, you can run a workflow from the Tower CLI. The command is very similar to the command to run a workflow from the CLI, but you need to:
...
If you have access to a Seqera Platform instance, previously known as Nextflow Tower, in which a Compute Environment has already been set up, you can run a workflow from the Tower UI. To do so, go to the "Launchpad" and click on the button "launch a run without configuration".
